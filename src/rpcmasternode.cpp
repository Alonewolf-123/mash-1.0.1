...
Value obfuscation(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "obfuscation <mashaddress> <amount>\n"
            "mashaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "obfuscation <mashaddress> <amount>\n"
            "mashaddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and will be rounded to the next 0.1" +
            HelpRequiringPassphrase());
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (strCommand == "winners") {
        ...
        if (params.size() >= 2) {
            try {
                nLast = std::atoi(params[1].get_str().c_str());
