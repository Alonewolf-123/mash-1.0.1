...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"mashaddress\"\n"
            "\nReturn information about the given mash address.\n"
            "\nArguments:\n"
            "1. \"mashaddress\"     (string, required) The mash address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"mashaddress\",   (string) The mash address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Mash address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are mash addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) mash address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"mashaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"mashaddress\"    (string, required) The mash address to use for the signature.\n"
