...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("MashMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("MashMiner : ProcessNewBlock, block not accepted");
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                ...
                if (!pwallet->IsLocked()) {
                    if (!pwallet->TopUpKeyPool()) {
                        LogPrintf("Error in MashMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                        return;
                    }
                }
                continue;
            ...
            LogPrintf("Running MashMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("MashMiner terminated\n");
        throw;
    }
    ...
    {
        LogPrintf("MashMiner runtime error: %s\n", e.what());
        return;
    }
